import { d as useSupabaseClient, c as useSupabaseUser } from './server.mjs';
...
const useBlog = () => {
  ...
  const uploadImage = async (file) => {
    try {
      ...
      const { data, error } = await supabase.storage.from("blog-images").upload(fileName, webpFile);
  ...
  const updateBlogPost = async (id, postData) => {
    try {
      const { data, error } = await supabase.from("blog_posts").update({ ...postData, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", id).select();
      if (error) throw error;
      return data[0];
    } catch (error) {
...
export { useBlog as u };
//# sourceMappingURL=useBlog-DLrcGbre.mjs.map
