        ...
        <div class="flex items-start">
          ...
          <div>
            ...
            <ul class="text-sm text-gray-600 space-y-1">
              ...
              <li>• Upload a 1200×630 px banner for best display across devices</li>
              <li>• Use the description editor's formatting to make content scannable</li>
              <li>• Consider tiered pricing for early birds or VIP attendees</li>
              <li>• Only add registration fields you truly need — shorter forms convert better</li>
            </ul>
          </div>
        </div>
      </div>
    ...
    <div v-if="showSuccessModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl max-w-md w-full overflow-hidden">
        ...
        <div class="p-6 flex flex-col space-y-3">
          <button @click="goToEventsList" class="w-full bg-[#ACBEA3] hover:bg-[#9BAD94] text-white font-medium py-3 px-4 rounded-lg transition-colors">View All Events</button>
          <button @click="createAnother" class="w-full border border-[#ACBEA3] text-[#ACBEA3] hover:bg-[#ACBEA3] hover:text-white font-medium py-3 px-4 rounded-lg transition-colors">Create Another Event</button>
        </div>
      </div>
    </div>
...
const uploadToCloudinary = async (file) => {
  ...
  try {
    ...
    bannerError.value = 'Banner upload failed. You can still create the event without a banner.'
    formData.value.banner_url = ''
  } finally {
...
const serializeFields = () =>
  formData.value.form_fields.map(({ _id, _expanded, optionsRaw, ...rest }) => ({
    ...rest,
    options: (rest.type === 'select' || rest.type === 'radio')
...
const handleCreateEvent = async () => {
  if (bannerUploading.value) {
    toast.add({ title: 'Please wait', description: 'Banner is still uploading.', color: 'yellow' })
    return
  }
  ...
  try {
    const payload = {
      ...
      pricing_tiers: formData.value.pricing_mode === 'tiered' && !formData.value.is_free
...
const createAnother = () => {
  showSuccessModal.value = false
  formData.value = {
    title: '', event_date: '', venue: '', description: '',
    admission_fee: 0, currency: '₦', is_free: true,
    pricing_mode: 'flat', pricing_tiers: [], form_fields: '', banner_url: '',
  }
...
.rich-editor :deep(h2) { font-size: 1.25rem; font-weight: 700; margin: 0.5rem 0; }
.rich-editor :deep(h3) { font-size: 1.1rem; font-weight: 600; margin: 0.4rem 0; }
...
.rich-editor :deep(ul) { list-style: disc; padding-left: 1.5rem; margin: 0.4rem 0; }
.rich-editor :deep(ol) { list-style: decimal; padding-left: 1.5rem; margin: 0.4rem 0; }
.rich-editor :deep(a) { color: #ACBEA3; text-decoration: underline; }
.rich-editor :deep(strong) { font-weight: 700; }
.rich-editor :deep(em) { font-style: italic; }
</style>